...
func SetCookieInfo(username string, cookieInfo *LoginResponse_Data_CookieInfo) error {
	...
	return localdb.SetJson(BilibiliUserCookieInfoKey(username), cookieInfo, opt)
}

func GetCookieInfo(username string) (cookieInfo *LoginResponse_Data_CookieInfo, err error) {
	err = localdb.GetJson(BilibiliUserCookieInfoKey(username), &cookieInfo)
	return
}

func ClearCookieInfo(username string) error {
	_, err := localdb.Delete(BilibiliUserCookieInfoKey(username), localdb.IgnoreNotFoundOpt())
	return err
}
